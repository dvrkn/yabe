...
pub fn sort_only_workflow(args: &Args) -> Result<(), Box<dyn Error>> {
    info!("Running in sort-only mode");

    let expanded_input_files = expand_and_filter_files(
    ...
    if !args.inplace {
        ensure_output_dir(&args.out_folder)?;
    }
...
pub fn run_legacy_main(mut args: Args) -> Result<(), Box<dyn Error>> {
